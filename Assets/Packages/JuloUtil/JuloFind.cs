	...
	public class JuloFind {
		...
		public static T singleton<T>(bool required = true) where T : Component {
			...
			if(all.Count == 1) {
				...
				if(required) {
					throw new NotFoundException("No object whith component " + typeof(T) + " found");
				} else {
					return null;
				}
			} else {
				throw new MoreThanOneException("More than one object whith component " + typeof(T) + " found");
			}
		}
		...
		private static T getComponent<T>(GameObject obj) where T : Component {
			...
			if(ret == null)
				throw new RequiredComponentException("Object " + obj + " hasn't component " + typeof(T));
		...
		private static T oneWithName<T>(string name, bool required = true) where T : Component {
			...
			if(all.Count == 0) {
				if(required) {
					throw new NotFoundException("No object named '" + name + "' found in scene");
				} else {
					return null;
				}
			} else if(all.Count > 1) {
				throw new MoreThanOneException("More than one object named '" + name + "' found in scene");
			} else {
				return all[0];
			}
		}
